...
int main(int argc, char **argv) {
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "harris_color_kernel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, err = krnl_vector_add.setArg(1, op_hcompute_hw_output_global_wrapper_stencil_write_pipe0_ocl_buf));
  ...
  OCL_CHECK(err, err = krnl_vector_add.setArg(2, op_hcompute_hw_input_global_wrapper_glb_stencil_1_read_pipe0_ocl_buf));
  ...
  OCL_CHECK(err, err = krnl_vector_add.setArg(3, op_hcompute_hw_input_global_wrapper_glb_stencil_2_read_pipe0_ocl_buf));
  ...
  OCL_CHECK(err, err = krnl_vector_add.setArg(4, op_hcompute_hw_input_global_wrapper_glb_stencil_3_read_pipe0_ocl_buf));
  ...
  OCL_CHECK(err, err = krnl_vector_add.setArg(5, op_hcompute_hw_input_global_wrapper_glb_stencil_4_read_pipe0_ocl_buf));
  ...
  OCL_CHECK(err, err = krnl_vector_add.setArg(6, op_hcompute_hw_input_global_wrapper_glb_stencil_5_read_pipe0_ocl_buf));
  ...
  OCL_CHECK(err, err = krnl_vector_add.setArg(7, op_hcompute_hw_input_global_wrapper_glb_stencil_read_pipe0_ocl_buf));

  uint64_t transfer_size = num_epochs*(0 / 1);
  OCL_CHECK(err, err = krnl_vector_add.setArg(8, transfer_size));
